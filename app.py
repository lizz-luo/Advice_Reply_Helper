...
HELP_OPTIONS = {
    ...
    "language": [
        ...
        {"value": "strong_words", "label": "⚡ Power words (e.g. sad → upset, good → wonderful, happy → delighted)"},
    ],
    "organisation": [
...
def get_focus_label(value: str) -> str:
    for opts in HELP_OPTIONS.values():
        for o in opts:
            if o["value"] == value:
                raw = o["label"]
                return raw.split("(")[0].replace("🎯", "").replace("💡", "").replace("🔍", "").replace("❤️", "").replace("💪", "").replace("🔄", "").replace("🤗", "").replace("🔗", "").replace("✏️", "").replace("⚡", "").replace("👋", "").replace("📨", "").replace("📄", "").replace("🌟", "").strip().replace("=", "").strip()
...
def build_prompt(writing, student_name, mode, help_values, custom_q):
    ...
    prompt = (
        ...
        "3. Use very simple English for beginner English learners, especially weaker learners. Use short sentences and easy words only.\n"
    ...
    if goals_block:
        prompt += f"=== CHECKLIST GOALS TO REVIEW ===\n{goals_block}\n\n"
    prompt += (
...
def format_feedback_output(text: str) -> str:
    ...
    table_lines = [line for line in table_part.splitlines() if line.strip().startswith("|")]
    clean_table = "\n".join(table_lines)
    ex = rest.strip()
    if "✏️ How to Make It Better" in ex:
        ex = ex.split("✏️ How to Make It Better", 1)[1].strip()
    ex = re.sub(r'(📌|Example\s+\d+:|❌|✅|💡)', r'\n\n\1', ex)
    ex = re.sub(r'\n{3,}', '\n\n', ex)
    ex = ex.replace("\n", "<br>")
    output = clean_table
    if ex.strip():
...
def download_log_html() -> bytes:
    ...
    rows = [
        ...
        "<style>body{font-family:Arial,sans-serif;max-width:900px;margin:0 auto;padding:24px;color:#0c1929;background:#f0f9ff;line-height:1.65}h1{text-align:center;color:#0369a1;margin-bottom:4px}.subtitle{text-align:center;color:#b45309;font-style:italic;font-weight:700;margin-bottom:24px}.info,.session{background:#fff;border:1px solid #bae6fd;border-radius:14px;padding:18px;margin-bottom:18px}.tag{display:inline-block;background:#e0f2fe;border-radius:999px;padding:4px 10px;margin:2px 6px 2px 0;font-size:12px}.sample{background:#f8fbff;border:1px solid #dbeafe;padding:12px 14px;border-radius:8px;white-space:pre-wrap;word-break:break-word;overflow-wrap:anywhere}table{width:100%;border-collapse:collapse;margin:12px 0;font-size:14px}th{background:#0ea5e9;color:#fff;padding:10px;text-align:left}td{padding:10px;border-bottom:1px solid #dbeafe;vertical-align:top}tr:nth-child(even) td{background:#f8fbff}</style></head><body>",
        "<h1>Advice Reply Helper</h1><div class='subtitle'>Learning Log</div>",
        "<div class='info'>",
    ...
    for i, entry in enumerate(history, 1):
        ...
        rows.append(f"<div><strong>Time:</strong> {escape_html(entry.get('timestamp',''))}</div>")
        rows.append(f"<div><strong>Mode:</strong> {escape_html(entry.get('mode_label',''))}</div>")
        if goals:
            rows.append("<div><strong>Checklist Goals:</strong> " + ", ".join(escape_html(g) for g in goals) + "</div>")
        if entry.get("custom_question"):
            rows.append(f"<div><strong>Custom Question:</strong> {escape_html(entry['custom_question'])}</div>")
        rows.append("<p><strong>My Email:</strong></p>")
        rows.append(f"<div class='sample'>{escape_html(entry.get('writing',''))}</div>")
        rows.append("<p><strong>AI Feedback:</strong></p>")
        rows.append(f"<div class='sample'>{entry.get('response','')}</div>")
        rows.append("</div>")
...
if st.session_state.pop("clear_writing_on_next_run", False):
    ...
    st.session_state["step2_confirmed"] = False
if st.session_state.pop("reset_after_step2_on_next_run", False):
    do_reset_after_step2()
if st.session_state.pop("reset_from_step3_on_next_run", False):
    do_reset_from_step3()
if st.session_state.pop("trigger_auto_download", False):
    st.session_state["save_log_requested"] = True

st.markdown("""
<style>
:root{--bg:#f0f9ff;--bg-soft:#f8fafc;--panel:#f3f4f6;--panel-border:#e2e8f0;--text:#0f172a;--muted:#475569;--accent:#0369a1;--accent-soft:#e0f2fe;--input-bg:#ffffff;--code-bg:#ffffff;}
@media (prefers-color-scheme: dark){
:root{--bg:#0f172a;--bg-soft:#111827;--panel:#1f2937;--panel-border:#334155;--text:#f8fafc;--muted:#cbd5e1;--accent:#7dd3fc;--accent-soft:#082f49;--input-bg:#111827;--code-bg:#0b1220;}
}
...
.panel,.hero{background:var(--panel);border:1px solid var(--panel-border);border-radius:18px;box-shadow:0 2px 12px rgba(14,165,233,0.06);color:var(--text);}
...
input, textarea, .stTextInput input, .stTextArea textarea, div[data-baseweb="input"] input, div[data-baseweb="textarea"] textarea{background-color:var(--input-bg) !important;color:var(--text) !important;}
...
.next-step-btn div button{background:linear-gradient(135deg,#0ea5e9,#0284c7) !important;color:#fff !important;border:none !important;font-weight:700 !important;}
...
@keyframes floaty{0%{transform:translateY(0px) rotate(0deg);}50%{transform:translateY(-8px) rotate(-4deg);}100%{transform:translateY(0px) rotate(0deg);}}
</style>
""", unsafe_allow_html=True)

st.markdown("<div class='hero'><div class='badge'>Advice Reply Helper</div><div class='hero-title-row'><span class='floating-emoji'>✉️</span><h1 style='margin:0 0 0.35rem 0;color:var(--text)'>Advice Reply Helper</h1><span class='floating-emoji'>🪄</span></div><p class='small-note' style='font-style:italic;font-weight:600'>Your Friendly Helper for Writing a Better Reply!</p></div>", unsafe_allow_html=True)

st.markdown("<div class='panel'>", unsafe_allow_html=True)
st.markdown("<h3>Step 1 About You</h3>", unsafe_allow_html=True)
...
step1_ok = all(st.session_state.get(k, "").strip() for k in ["student_name", "student_class", "student_number"])
...
step1ok = st.session_state.get("step1_confirmed", False)

st.markdown("<div class='panel'>", unsafe_allow_html=True)
st.markdown("<h3>Step 2 Your Advice Reply Email</h3>", unsafe_allow_html=True)
if not step1ok:
    st.info("Please complete Step 1 first.")
st.text_area("", key="writing_input", placeholder="Paste your whole email here, or just the part you want feedback on...", height=220, disabled=not step1ok)
wc = word_count(st.session_state.get("writing_input", ""))
st.caption(f"{wc} word{'s' if wc != 1 else ''}")
if st.button("Next Step", use_container_width=True, key="step2_next", disabled=not step1ok):
...
step2ok = st.session_state.get("step2_confirmed", False)

st.markdown("<div class='panel'>", unsafe_allow_html=True)
st.markdown("<h3>Step 3 What Do You Need Help With?</h3>", unsafe_allow_html=True)
if not step2ok:
    st.info("Please complete Step 2 first.")
mode = st.radio("Choose a help category", options=list(HELP_OPTIONS.keys()), format_func=lambda x: x.title(), key="selected_mode", horizontal=True, disabled=not step2ok)
if step2ok and mode:
    st.markdown(f"<p class='small-note'>{MODE_DESCRIPTIONS[mode]}</p>", unsafe_allow_html=True)
st.markdown("</div>", unsafe_allow_html=True)

step3ok = step2ok and bool(mode)
allgoaloptions = [item["value"] for item in HELP_OPTIONS.get(mode, [])] if step3ok else []
goalmap = {item["value"]: item["label"] for item in HELP_OPTIONS.get(mode, [])}
cur = st.session_state.get("help_values", [])
st.session_state["help_values"] = [v for v in cur if v in allgoaloptions]

st.markdown("<div class='panel'>", unsafe_allow_html=True)
st.markdown("<h3>Step 4 Pick Your Goals</h3>", unsafe_allow_html=True)
if not step3ok:
    st.info("Please complete Step 2 and choose a category first.")
else:
    st.caption("Tick one or more goals you want help with.")
    st.multiselect("What would you like feedback on?", options=allgoaloptions, format_func=lambda x: goalmap.get(x, x), key="help_values", disabled=not step3ok)
st.markdown("</div>", unsafe_allow_html=True)

st.markdown("<div class='panel'>", unsafe_allow_html=True)
st.markdown("<h3>Step 5 Ask Your Own Question <em>optional</em></h3>", unsafe_allow_html=True)
st.text_area("Do you have a question about your email?", key="custom_question", placeholder="e.g. Does my advice sound kind? Did I show I care?", height=90, disabled=not step3ok)
st.markdown("</div>", unsafe_allow_html=True)

submit = st.button("Get Help", type="primary", use_container_width=True, disabled=not step3ok)
if submit:
    writing = st.session_state.get("writing_input", "").strip()
    customq = st.session_state.get("custom_question", "").strip()
    hvs = st.session_state.get("help_values", [])
    md = st.session_state.get("selected_mode", "content")
    name = st.session_state.get("student_name", "").strip()
    if not writing or len(writing) < 10:
        st.error("Please type or paste your advice reply email first, with at least a few sentences.")
    elif not hvs and not customq:
        st.error("Please select at least one checklist goal, or type your own question.")
    elif llm_detect_write_for_me(customq):
        st.warning("I can't write or finish your email for you. Try your best first, then I will give you tips to improve it.")
    else:
        prompt = build_prompt(writing, name, md, hvs, customq)
        try:
            ...
            st.session_state["interaction_history"].append({
                ...
                "help_goals": [get_focus_label(v) for v in hvs],
...
if st.session_state.get("feedback_text"):
    st.markdown("<div class='panel'>", unsafe_allow_html=True)
    st.markdown("<h3>Your Feedback</h3>", unsafe_allow_html=True)
...
if st.session_state.get("interaction_history"):
    ...
    with nx1:
        if st.button("Try Other Goals", use_container_width=True, help="Keep the same email, choose different goals"):
            st.session_state["reset_from_step3_on_next_run"] = True
            st.rerun()
    with nx2:
        if st.button("Check a New Part of My Email", use_container_width=True, help="Go back to Step 2, clear the email box, and reset later steps"):
...
if st.session_state.get("save_log_requested", False):
    ...
    st.download_button(
        ...
        file_name=f"LearningLog_{st.session_state.get('student_name') or 'Student'}.html",
        mime="text/html",
...
with st.expander("Session History", expanded=st.session_state.get("session_history_expanded", False)):
    ...
    if not history:
        ...
        for i, item in enumerate(reversed(history), 1):
            ...
            st.markdown(f"<span class='history-meta'>{item.get('timestamp','')}</span> <span class='history-meta'>{item.get('mode_label','')}</span>", unsafe_allow_html=True)
            for g in item.get("help_goals", []):
            ...
            st.text_area(f"Writing sample {idx}", value=item.get("writing", ""), height=180, disabled=True, label_visibility="collapsed", key=f"history_writing_{idx}")
            st.markdown("**Feedback**")
            st.markdown(item.get("response", ""), unsafe_allow_html=True)
            st.markdown("</div>", unsafe_allow_html=True)
