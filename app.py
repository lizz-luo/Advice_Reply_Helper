...
st.markdown(
    ...
    unsafe_allow_html=True,
)def post_process_table(text: str) -> str:
    return text.strip()


def post_process_examples(text: str) -> str:
    text = text.strip()
    for m in ["❌", "✅", "💡", "📌", "Example 1:", "Example 2:"]:
        text = text.replace(m, f"
...
def get_ai_feedback_table(prompt: str) -> str:
    client = get_groq_client()
    completion = client.chat.completions.create(
        model="llama-3.3-70b-versatile",
        temperature=0.1,
        messages=[
            {
                "role": "system",
                "content": "You are a helpful encouraging writing coach for primary school students aged 10-11. Output only the feedback table in concise markdown. Do not include examples or extra headings.",
            },
            {"role": "user", "content": prompt},
        ],
    )
    return post_process_table(completion.choices[0].message.content)


def get_ai_feedback_examples(prompt: str) -> str:
    client = get_groq_client()
    completion = client.chat.completions.create(
        model="llama-3.3-70b-versatile",
        temperature=0.1,
        messages=[
            {
                "role": "system",
                "content": "You are a helpful encouraging writing coach for primary school students aged 10-11. Output only the examples section in concise markdown. Put each marker on its own line and do not merge lines.",
            },
            {"role": "user", "content": prompt},
        ],
    )
    return post_process_examples(completion.choices[0].message.content)
